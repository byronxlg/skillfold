...
The 20 s video on the landing page and in the README is generated by the `/brag` skill
(Hyperframes). To regenerate: run `/brag` in this repo; it writes `brag/` (plan, brief,
`composition/`, `brag.mp4`, `brag.jpg`). Copy `brag.mp4` and `brag.jpg` to `site/assets/` and
merge; `docs.yml` ships them with the site. The README links the poster to the site copy
because GitHub does not play external mp4 inline. Keep the mp4 under about 6 MB.
