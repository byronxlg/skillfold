...
import { assignColor, generateAgents, generateGeminiAgents, generateRunCommand, generateTeamBootstrap } from "./agent.js";
...
describe("generateTeamBootstrap", () => {
  it("generates bootstrap prompt with team structure", () => {
    const config = makeConfig();
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.equal(result.name, "start-team");
    assert.ok(result.content.includes("Create an Agent Team"));
    assert.ok(result.content.includes("test-pipeline"));
    assert.ok(result.content.includes("Team Structure"));
  });

  it("lists all non-orchestrator teammates", () => {
    const config = makeConfig();
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.ok(result.content.includes("**planner**"));
    assert.ok(result.content.includes("**engineer**"));
    assert.ok(result.content.includes("**reviewer**"));
    // Orchestrator should not be listed as a teammate
    assert.ok(!result.content.includes("- **orchestrator**"));
  });

  it("includes task sequence with reads and writes", () => {
    const config = makeConfig();
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.ok(result.content.includes("Task Sequence"));
    assert.ok(result.content.includes("`state.plan`"));
    assert.ok(result.content.includes("`state.code`"));
    assert.ok(result.content.includes("`state.review`"));
  });

  it("includes shared state table", () => {
    const config = makeConfig();
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.ok(result.content.includes("Shared State"));
    assert.ok(result.content.includes("| plan | string |"));
    assert.ok(result.content.includes("| code | string |"));
  });
  ...
  it("includes coordination instructions", () => {
    ...
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.ok(result.content.includes("Coordination"));
    assert.ok(result.content.includes(".claude/agents/"));
  });

  it("returns null when config has no team", () => {
    const config = makeConfig({ team: undefined });
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.equal(result, null);
  });

  it("includes conditional routing in task sequence", () => {
    const graph: Graph = {
      nodes: [
        { skill: "engineer", reads: [], writes: ["state.code"] },
        {
          ...
          writes: ["state.review"],
          then: [
            { when: "review == approved", to: "end" },
            { when: "review == rejected", to: "engineer" },
          ],
        },
      ],
    };

    const config = makeConfig({ team: { orchestrator: "orchestrator", flow: graph } });
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.ok(result.content.includes("Routing:"));
    assert.ok(result.content.includes("review == approved"));
    assert.ok(result.content.includes("review == rejected"));
  });

  it("includes provenance comment", () => {
    const config = makeConfig();
    const result = generateTeamBootstrap(config, "1.0.0", "test.yaml");
    assert.ok(result);
    assert.ok(result.content.includes("Generated by skillfold v1.0.0"));
  });
});
...
describe("generateGeminiAgents", () => {
  ...
  it("agent files contain Gemini frontmatter with name and description", () => {
    ...
    assert.ok(engineer.content.includes("name: engineer"));
    assert.ok(engineer.content.includes("description: Writes code."));
    // Gemini agents do not include color or model: inherit by default
    assert.ok(!engineer.content.includes("color:"));
    assert.ok(!engineer.content.includes("model:"));
  });
  ...
  it("maps agentConfig model to frontmatter", () => {
    const config = makeConfig({
      skills: {
        planning: { path: "./skills/planning" },
        planner: {
  ...
  it("maps agentConfig maxTurns to max_turns in frontmatter", () => {
    const config = makeConfig({
      skills: {
        planning: { path: "./skills/planning" },
        planner: {
    ...
    assert.ok(planner.content.includes("max_turns: 20"));
    // Should not use camelCase
    assert.ok(!planner.content.includes("maxTurns:"));
  });

  it("maps agentConfig tools to frontmatter", () => {
    const config = makeConfig({
      skills: {
        planning: { path: "./skills/planning" },
        planner: {
          compose: ["planning"],
          description: "Plans.",
          agentConfig: { tools: ["shell", "file_read"] },
        },
      },
      team: undefined,
    });
    ...
    assert.ok(planner.content.includes("- shell"));
    assert.ok(planner.content.includes("- file_read"));
  });

  it("orchestrator agent has special heading and includes execution plan", () => {
    const config = makeConfig();
    const bodies = new Map<string, string>();
    bodies.set("orchestrator", "Orchestrate.");
    ...
    assert.ok(orch.content.includes("# orchestrator (orchestrator)"));
    assert.ok(orch.content.includes("lead orchestrator agent"));
    // Orchestrator should include execution plan
  ...
  it("does not emit model when agentConfig has no model", () => {
    const config = makeConfig({
      skills: {
        planning: { path: "./skills/planning" },
        planner: {
    ...
    assert.ok(fmMatch);
    const fmLines = fmMatch[1].split("\n").map((l) => l.split(":")[0].trim());
    assert.deepEqual(fmLines, ["name", "description"]);
  });
});
