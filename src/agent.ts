...
/** Format Gemini agent markdown with Gemini-specific YAML frontmatter. */
function formatGeminiAgentMarkdown(
  ...
  if (agentConfig) {
    // Map agentConfig fields to Gemini frontmatter (snake_case)
    if (agentConfig.model !== undefined) {
      frontmatter.push(`model: ${agentConfig.model}`);
    }
    if (agentConfig.tools !== undefined) {
      const toolsYaml = stringify({ tools: agentConfig.tools }, { lineWidth: 0 }).trimEnd();
      frontmatter.push(toolsYaml);
    }
    if (agentConfig.maxTurns !== undefined) {
      frontmatter.push(`max_turns: ${agentConfig.maxTurns}`);
    }
    // Pass through Gemini-native fields
    if (agentConfig.timeout_mins !== undefined) {
      frontmatter.push(`timeout_mins: ${agentConfig.timeout_mins}`);
    }
    if (agentConfig.temperature !== undefined) {
      frontmatter.push(`temperature: ${agentConfig.temperature}`);
    }
  }
...
/** Generate Gemini agent files under agents/ subdirectory. */
