...
describe("compile with goose target", () => {
  ...
  it("outputs .goosehints at project root", () => {
    tmpDir = makeTmpDir();
    const outDir = join(tmpDir, ".goose");
    ...
    assert.equal(results.length, 1);
    assert.equal(results[0].name, "goosehints");
    assert.equal(results[0].path, join(tmpDir, ".goosehints"));
    assert.ok(existsSync(results[0].path));
  });

  it("contains provenance header and skill body", () => {
    tmpDir = makeTmpDir();
    const outDir = join(tmpDir, ".goose");
    ...
    const content = readFileSync(join(tmpDir, ".goosehints"), "utf-8");
    assert.ok(content.includes("<!-- Generated by skillfold v0.0.0"));
    assert.ok(content.includes("# test"));
    assert.ok(content.includes("## bundle"));
    assert.ok(content.includes("A bundle skill."));
  ...
  it("includes orchestrator plan when team flow exists", () => {
    ...
    const content = readFileSync(join(tmpDir, ".goosehints"), "utf-8");
    assert.ok(content.includes("# Orchestrator: goose-test"));
    assert.ok(content.includes("## worker"));
  });
});
...
describe("compile with roo-code target", () => {
  ...
  it("outputs skills in mode-specific rule directories and .roomodes", () => {
    tmpDir = makeTmpDir();
    const outDir = join(tmpDir, "dist");
    ...
    assert.equal(ruleResult.path, join(outDir, "rules-bundle", "bundle.md"));
  ...
  it("generates standalone orchestrator rule when team has no orchestrator key", () => {
    ...
    assert.ok(orchResult);
    assert.equal(orchResult.path, join(outDir, "rules-orchestrator", "orchestrator.md"));
...
describe("compile with junie target", () => {
  ...
  it("outputs skills and single AGENTS.md", () => {
    tmpDir = makeTmpDir();
    const outDir = join(tmpDir, ".junie");
    ...
    assert.equal(agentsResult.path, join(outDir, "AGENTS.md"));
  });

  it("AGENTS.md is plain markdown with all agent sections", () => {
    tmpDir = makeTmpDir();
    const outDir = join(tmpDir, ".junie");
    ...
    assert.equal(frontmatterLines.length, 0, "Junie AGENTS.md should not have YAML frontmatter");
  });
  ...
  it("includes orchestrator plan when team flow exists", () => {
    ...
    assert.ok(agentsResult);
    assert.equal(agentsResult.path, join(outDir, "AGENTS.md"));
    assert.ok(existsSync(agentsResult.path));
    const content = readFileSync(agentsResult.path, "utf-8");
